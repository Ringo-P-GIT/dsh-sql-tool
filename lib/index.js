...
function processSql(text, mode, options) {
  const engine = getEngine()
  const funcList = loadFuncList()
  if (engine && typeof engine.process === 'function') {
    const result = engine.process({ text, mode: options?.replacePairs ? 'replace' : mode, funcList: funcList || undefined, validate: mode !== 'format' || undefined, replacePairs: options?.replacePairs, order: options?.order, compact: options?.compact })
    return { formatted: result.formatted ?? text, formatSafe: result.formatSafe, diagnostics: result.diagnostics ?? [], macros: result.macros ?? [], diff: result.diff, sql: result.sql, applied: result.applied, stable: result.stable }
  }
...
async function handleApiRequest(req, res) {
  ...
  const { text, language = 'sql', mode = 'format', options = {} } = input
  ...
  try {
    const result = language === 'json' ? processJson(text, mode) : processSql(text, mode, options)
...
export default function apply(ctx) {
  ...
  try {
    tools.register({
      name: 'sql_tool',
      description: 'SQL(含@宏)与JSON的格式化/校验/自动修复工具。支持@form/@sqlvalue/@nullValue/@getDeptCode/@sqlset/@processId等宏。',
      parameters: { type: 'object', properties: { text: { type: 'string', description: '待处理的SQL或JSON文本' }, language: { type: 'string', description: '语言类型', default: 'sql' }, mode: { type: 'string', description: 'format/validate/replace', default: 'format' }, options: { type: 'object', description: '可选参数', properties: { compact: { type: 'boolean' }, replacePairs: { type: 'array', items: { type: 'object', properties: { find: { type: 'string' }, value: { type: 'string' }, quoted: { type: 'boolean' } } } }, order: { type: 'string', default: 'replace-then-format' } } } }, required: ['text'] },
      output: { schema: { type: 'object', properties: { formatted: { type: 'string' }, formatSafe: { type: 'boolean' }, diagnostics: { type: 'array', items: { type: 'object', properties: { line: { type: 'number' }, col: { type: 'number' }, level: { type: 'string' }, rule: { type: 'string' }, message: { type: 'string' } } } }, macros: { type: 'array', items: { type: 'object', properties: { text: { type: 'string' }, params: { type: 'number' }, count: { type: 'number' } } } } } },
        render: (args, value) => { const parts = []; if (value.formatted) { parts.push('```sql'); parts.push(value.formatted); parts.push('```') }; if (value.diagnostics?.length) for (const d of value.diagnostics) parts.push(`[${d.level.toUpperCase()}] ${d.rule}: ${d.message} (${d.line}:${d.col})`); return parts.join('\n') } },
      execute: (args) => {
        const { text, language = 'sql', mode = 'format', options = {} } = args
        ...
        return processSql(text, mode, options)
      }
    })
